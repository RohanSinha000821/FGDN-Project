...
def load_checkpoint(project_root: Path, atlas: str, num_folds: int, fold: int, checkpoint_type: str):
    ...
    ckpt_name = "best_fgdn_weighted.pt" if checkpoint_type == "best" else "last_fgdn_weighted.pt"
...
def rebuild_templates_from_checkpoint_split(outer_train_dataset, checkpoint):
    ...
    if split_info is None:
        raise ValueError("Checkpoint missing split_info; cannot reconstruct weighted templates.")
    ...
    asd_edge_index, asd_edge_weight, _, asd_theta = build_weighted_knn_graph(asd_mean_fc, k=template_k)
    hc_edge_index, hc_edge_weight, _, hc_theta = build_weighted_knn_graph(hc_mean_fc, k=template_k)
...
def evaluate(model, loader, device):
    y_true, y_pred, y_prob, subject_ids = [], [], [], []
...
def main():
        ...
        json.dump(
            {
                ...
                "template_weighting": checkpoint.get("split_info", {}).get("template_weighting", None),
    ...
    print(f"ASD weight range          : [{asd_edge_weight_np.min():.6f}, {asd_edge_weight_np.max():.6f}]")
    print(f"HC weight range           : [{hc_edge_weight_np.min():.6f}, {hc_edge_weight_np.max():.6f}]")
